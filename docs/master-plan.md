...
**M6.3 — chat identity, lifecycle, and presentation: delivered, then corrected by M6.3.1** (`docs/plans/m06-3-chat-identity-lifecycle-presentation-plan-v1.md`, `docs/adr/0024-...md`). Adds: a required visitor display name (1-80 chars, encrypted at rest, write-once) collected before the first message, feeding the Telegram topic title and a one-time first-message context header; a reload-safe `display_name_required` REST contract; three compiled static presentation presets (classic/modern/minimal, modern default) with a documented `.ut-chat-widget` CSS custom-property/selector contract, always honouring the visitor's own reduced-motion preference; configurable participant labels (default You/Support); 30-day inactivity auto-archival via the existing status-transition map; and destination-list hygiene separating chat-widget-created Telegram topics from manually configured destinations in the Bots tab. `db_version` 14 → 15. The Product Owner rejected the visitor-entered-name onboarding model after technical closure; M6.3.1 (below) replaces it.

**M6.3.1 — authenticated chat access and UX redesign: delivered** (`docs/plans/m06-3-1-authenticated-chat-access-ux-plan-v1.md`, `docs/adr/0025-...md`). Corrects M6.3: chat now requires an authenticated WordPress user (a logged-out visitor sees only sign-in/create-account links); the visitor-entered display name is replaced by a server-derived name from the WordPress account (fixed generic fallback when empty), removing the `display_name_required` contract and the name-entry form entirely. Conversations gain a persistent `owner_user_id` and a database-enforced one-active-conversation-per-owner-per-bot guarantee (a generated, unique-indexed column — never a request-timing assumption). Every conversation REST route now explicitly requires a WordPress cookie session and nonce (additive to the existing bearer secret), plus an ownership match on message/poll. A new `GET /conversations/mine` route resumes a returning authenticated visitor's conversation across browsers/sessions. Opening the panel creates nothing; the first Send invisibly starts and posts the conversation, and the "Start chat"/"End conversation" controls are removed. Account deletion revokes the conversation's secret and clears the ownership link. The default style preset is now `theme` (generic WordPress/WooCommerce CSS-variable fallback chains, no hard-coded color), and the message log respects a visitor's manual upward scroll with a "New messages" affordance. `db_version` 15 → 16. Pre-existing M05-M06.3 (ownerless) conversations remain in the database under existing retention but are no longer reachable through the widget. Still explicitly deferred: email collection/transcript delivery (proposed M06.4), logged-in WooCommerce order context (proposed M06.5), operator conversation UI, manual delete, custom-CSS editor, external stylesheet URLs, chat profiles/targeting, business hours, pre-chat form, live preview.
...
* **Legacy export boundary pin (ADR-0039):** Support Chat ADR-0008 commit `7546d43be66f8e3b2f179f03a1c81c9aadef59db` —
  ...
  (narrow, versioned, in-process, WP-CLI-only `LegacyExportServiceV1`; implemented, Product Owner accepted — see ADR-0039 for the authorised follow-up scope)
* **Legacy binding preparation boundary pin (ADR-0041):** Support Chat ADR-0009 commit `590b53ba898aa4054ec65c65965c152a3612149b` —
  ...
  (narrow, versioned, in-process, WP-CLI-only `LegacyBindingImportServiceV1`; a new non-routing `prepared` binding status; a lock-scoped quiescence assertion; implemented, Product Owner accepted — see ADR-0041 for the authorised follow-up scope)
* **Final-cutover follow-up pin (ADR-0042):** Support Chat ADR-0010 (pin SHA recorded in ADR-0042 once Support Chat's PR merges) —
  (cutover-orchestration state machine; monotonic-CAS activation/compensation saga; cohort-aware deferred-update replay; this plugin's own incident record; `maybe_mark_topic_unavailable()` cross-talk resolution — **documentation frozen only, no implementation** — see ADR-0042 for the authorised follow-up scope)
* Sequence: `SC-M00–M02` → UT Adapter M1 → **ADR-0038 signed-client follow-up (implemented)** → **ADR-0039 legacy export boundary follow-up (implemented, PO accepted)** → `SC-M03` work packages 3–4 (implemented, PO accepted) → `SC-M03` work package 2 (implemented, PO accepted, ADR-0040) → **ADR-0041 legacy binding preparation follow-up (implemented, PO accepted)** → `SC-M03` work package 5 (implemented, PO accepted) → **ADR-0042 final-cutover follow-up (implemented, PO accepted — WP10)** → **disposable DEV rehearsal (runbook frozen; Approval A recorded; Tier 1 attempted 2026-08-27 → halted by finding F1 — a production `channel_case_ref`/`binding_uuid` resolution gap under separate ADR review; harness validated, no production code changed; Tier 2 blocked on B1/B2/F1 and pending Approval B)** → `SC-M03` final cutover (future, separately-approved implementation) → `SC-M04`
