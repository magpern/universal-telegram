...
* Sequence: `SC-M00–M02` → UT Adapter M1 → **ADR-0038 signed-client follow-up (implemented)** → **ADR-0039 legacy export boundary follow-up (implemented, PO accepted)** → `SC-M03` work packages 3–4 (implemented, PO accepted) → `SC-M03` work package 2 (implemented, PO accepted, ADR-0040) → **ADR-0041 legacy binding preparation follow-up (implemented, PO accepted)** → `SC-M03` work package 5 (implemented, PO accepted) → **ADR-0042 final-cutover follow-up (implemented, PO accepted — WP10)** → **disposable DEV rehearsal (runbook frozen; Tier 1 authorized — Approval A recorded; Tier 2 blocked on B1/B2 and pending Approval B)** → `SC-M03` final cutover (future, separately-approved implementation) → `SC-M04`
