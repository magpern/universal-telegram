...
**Cross-repo sequence:** `SC-M00–M02` → **UT Adapter M1** → **ADR-0038 signed-client follow-up (implemented)** → **ADR-0039 legacy export boundary follow-up (implemented, PO acceptance pending)** → `SC-M03` work packages 3–4 (implemented, PO accepted) → `SC-M03` work package 2 (implemented, PO accepted, ADR-0040) → **ADR-0041 legacy binding preparation follow-up (pinned, not yet implemented)** → `SC-M03` work package 5 → `SC-M03` work packages 6–8 → `SC-M04`.
