...
**Planning-only:** [DEV rehearsal runbook **v2**](../plans/sc-m03-final-cutover-dev-rehearsal-plan-v2.md) supersedes v1, pins the immutable Product-Owner-approved Tier 1 execution baselines UT `6eed0228286e84b4e56e0119f242b483f138a58e` / SC `4f833c3344c3cff2adcc0227f93832c0c3a4427a` (operators fetch origin, verify these exact commits exist, and check them out before execution; runtime trees byte-identical to the F1 implementation commits `7d4cc4f` / `9144cb1`, documentation only added since; future documentation merges must not alter the baseline without a new PO approval), and revises only the F1-invalidated portions (wire identity, Run 1 handoff fixture/assertions, the exhaustive fail-closed classifier referenced by Runs 2 and 3, a new Run 1 F1-correction gate, new Run 3 fail-closed incident scenarios); all v1 safety boundaries, evidence/redaction/teardown requirements, the Tier 1/Tier 2 distinction, and blockers B1–B5 are carried forward. F1 is **no longer a code blocker**. The Approval A addendum ([`docs/closure/sc-m03-final-cutover-dev-rehearsal-tier1-approval-addendum.md`](../closure/sc-m03-final-cutover-dev-rehearsal-tier1-approval-addendum.md)) is **RECORDED / Product Owner accepted 2026-08-28**, and **the single authorised Tier 1 re-attempt was executed on 2026-08-28 and PASSED** on both supported WP/PHP variants at the immutable execution baseline SHAs — closure [`docs/closure/sc-m03-final-cutover-dev-rehearsal-tier1-reattempt-closure.md`](../closure/sc-m03-final-cutover-dev-rehearsal-tier1-reattempt-closure.md) (dual-plugin interop `OK (47 tests, 722 assertions)` each variant; F1-correction gate held with a real `legacy-bind` binding; fail-closed classifier confirmed; disposable harness only, zero Telegram traffic, teardown proven). **Tier 1 is complete and the addendum's one-time authorisation is consumed.** The **Tier 2 prerequisite design is now frozen** — [`docs/plans/sc-m03-final-cutover-tier2-dev-rehearsal-prerequisites-plan-v1.md`](../plans/sc-m03-final-cutover-tier2-dev-rehearsal-prerequisites-plan-v1.md) (primary; Support Chat companion + decision-record Addendum E in that repo) specifies B1 (isolated `scm03rehearsal` instance sharing nothing with `dev.biopentra.eu` or `/opt/biopentra/{dev,data}/*`; public listeners unchanged; B1 verification gate), B2 (dedicated non-production Telegram bot/supergroup/topics; token encrypted only in the rehearsal DB; full webhook + `@BotFather` revocation lifecycle; B2 verification gate), the Tier 2 operator sequence (real WP-Cron/Action Scheduler, Redis, authenticated webhook ingress, real topic-lifecycle service messages, real `sendMessage`, real `409 quiescence_active`), and a **proposed, unsigned Approval B** (exactly one Tier 2 rehearsal at the immutable baselines, after B1/B2 are provisioned and independently verified). It **provisions nothing, creates no Telegram resource, records no Approval B, and executes no Tier 2.** Four-phase separation: planning → provisioning → Approval B recording → one-time execution. Tier 2 stays blocked on B1 and B2; a second Tier 1 attempt needs a new Product Owner approval.

**CLOSED by [ADR-0044](../adr/0044-universal-telegram-transport-only-retire-legacy-chat-and-cutover.md) (2026-08-28).** The Product Owner has decided Universal Telegram becomes a **Telegram transport / Support Chat adapter plugin only**: its legacy website chat is **retired and discarded, not migrated**, so the entire SC-M03 migration/cutover track (export, binding preparation, quiescence, cutover, deferred-update replay, cohort activation, incident machinery) is removed. There is **no cutover and no Tier 2 rehearsal**. The Tier 1 re-attempt closure and the Tier 2 prerequisites plan are retained as historical records; the proposed Approval B is withdrawn unsigned. Implementation: [`docs/plans/ut-transport-only-retire-legacy-chat-plan-v1.md`](../plans/ut-transport-only-retire-legacy-chat-plan-v1.md); closure on completion: `docs/closure/ut-transport-only-legacy-chat-retirement-closure.md`. ADR-0039/0040/0041/0042/0043 → Superseded by ADR-0044.
