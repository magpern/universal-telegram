...
**Baselines this runbook pins (freshly fetched `origin/main`, both HEAD — these include the F1
correction and its closure):**

| Repository | Pinned SHA | Notes |
|---|---|---|
| `magpern/universal-telegram` | `32f17ea904a33cdd1f9b0225ba9638f95a09d883` | Plugin version `0.19.0`; schema `target_version()` `36` (unchanged by F1). Contains the F1 runtime correction (`7d4cc4f`) and its closure. |
| `magpern/universal-support-chat` | `5d81b5b7795ee50f3a79e535a483d7677b36d1c0` | Plugin version `0.6.0`; `universal_support_chat_db_version` `11` (unchanged by F1). Contains the F1 comment corrections (`9144cb1`) and its closure. |
...
|---|---|---|
| A1 | The rehearsal env's checked-out plugin SHAs equal the **v2** baselines (`32f17ea…` / `5d81b5b…`) and running schema is UT `36` / SC `11`. | `git rev-parse HEAD`; `wp eval` on `Migrator::target_version()` / `get_option('universal_support_chat_db_version')`. |
| A3 | Whether `cutover begin` preflight enforces "mapping-complete on Support Chat's side" and "no blocking incident," or only the local `prepared` binding. UT `CutoverActivationService::preflight()` checks only for a `prepared`-status binding per candidate. | Read `CutoverActivationService::preflight()` at `32f17ea…`; drive a candidate whose SC map row is not `migrated` and observe whether `begin` refuses it. |
...
2. Both checkouts `git rev-parse HEAD` == the **v2** baselines (`32f17ea…` / `5d81b5b…`); `git status` clean; the checkouts are the throwaway pair, never `/opt/biopentra/dev/*`. (A1)
...
1. **Preconditions** (`00-preconditions/`): the signed Approval A addendum reference;
   `git rev-parse HEAD` == `32f17ea904a33cdd1f9b0225ba9638f95a09d883` (UT) and
   `5d81b5b7795ee50f3a79e535a483d7677b36d1c0` (SC), `git status` clean; `docker compose config`;
...
SHAs updated to the v2 baselines (`32f17ea904a33cdd1f9b0225ba9638f95a09d883` /
`5d81b5b7795ee50f3a79e535a483d7677b36d1c0`).
