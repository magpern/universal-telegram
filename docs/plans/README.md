...
- `docs/plans/sc-m03-final-cutover-dev-rehearsal-plan-v2.md` (current; primary operator runbook, superseding v1. Pins the current `origin/main` heads UT `33b042f` / SC `2000eaf` — `src/`+`tests/` identical to the F1 correction (`7d4cc4f` / `9144cb1`), only `docs/` added since the F1 closure (`32f17ea` / `5d81b5b`). Revises only the F1-invalidated parts of v1 — pinned baselines, the `channel_case_ref` wire identity (now the Support Chat conversation UUID; `binding_uuid` off the wire), Run 1's handoff fixture/assertions (real distinct `binding_uuid`), and the exhaustive fail-closed replay classifier (`unresolved_case_reference` / `handoff_rejected`) referenced by Runs 2 and 3; adds a Run 1 F1-correction gate and Run 3 fail-closed incident scenarios. All v1 safety boundaries, evidence/redaction/teardown requirements, the Tier 1/Tier 2 distinction, and blockers B1–B5 carried forward. **Authorizes nothing** — a Tier 1 re-attempt needs the **proposed, unsigned** Approval A addendum `docs/closure/sc-m03-final-cutover-dev-rehearsal-tier1-approval-addendum.md`; Tier 2 stays blocked on B1/B2 and pending Approval B; no rehearsal has run under v2)
