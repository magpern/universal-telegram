...
final class RetentionCleanupTest extends WP_UnitTestCase {
	...
	public function test_operational_summary_runs_older_than_90_days_are_deleted(): void {
		global $wpdb;
		...
		$remaining = $wpdb->get_col( "SELECT summary_date FROM {$table}" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
		$this->assertNotContains( '2020-01-01', $remaining );
		$this->assertContains( gmdate( 'Y-m-d' ), $remaining );
	}

	public function test_operational_summary_ai_drafts_older_than_30_days_are_deleted(): void {
		global $wpdb;
		...
		$remaining = $wpdb->get_col( "SELECT summary_run_id FROM {$table}" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
		$this->assertNotContains( '1', $remaining );
