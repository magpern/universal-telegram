...
final class ConversationTopicEligibilityTest extends WP_UnitTestCase {
	...
	private function archived_with_topic( ConversationRepository $conversations, DestinationRepository $destinations, string $chat_id = '-100elig', int $thread = 57 ): array {
	...
	public function test_shared_destination_id_is_ineligible_and_purge_keeps_dest(): void {
		[ $conversations, $destinations, $eligibility ] = $this->repos();
		[ $first, $destination ] = $this->archived_with_topic( $conversations, $destinations, '-100share', 90 );
